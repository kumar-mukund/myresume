...
// Ship the downloads-admin viewer alongside the app — nothing sensitive in
// the page itself (it's gated by list-downloads.mjs checking ADMIN_PASSWORD).
const ADMIN_SRC = path.join(__dirname, 'admin.html');
const ADMIN_OUT = path.join(OUT_DIR, 'admin.html');
if (fs.existsSync(ADMIN_SRC)) {
  fs.copyFileSync(ADMIN_SRC, ADMIN_OUT);
  console.log('Wrote', ADMIN_OUT);
}
